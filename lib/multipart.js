...
function Multipart(data) {
	...
	this.on('uploadId', function (uploadId) {
...
Multipart.prototype._getUploadId = function _getUploadId() {
	var params,
		_this = this;
...
Multipart.prototype._processFile = function _processFile() {
	var _this = this;

	fs.exists(this.config.file, function (exists) {
		var stream;
...
Multipart.prototype._processStream = function _processStream() {
	var _this = this;
...
Multipart.prototype._handleStream = function _handleStream() {
	var _this = this;

	this.config.stream.on('data', function (data) {
	...
	this.config.stream.on('error', function (err) {
		_this.emit('error', err);
		_this.abort();
	});

	this.config.stream.on('end', function () {
		if (_this.buffer) {
			_this._pushPart(_this.buffer);
			_this.buffer = false;
		}
...
Multipart.prototype._pushPart = function _pushPart(data, route) {
	(this.pending || this.queue).push({partId: ++this.numParts, data: data});
};

Multipart.prototype._handleComplete = function _handleComplete() {
	var params, req,
		_this = this;
	...
	req = this.S3.completeMultipartUpload(params, function (err, data) {
		if (err) {
			_this.emit('error', err);
			_this.abort();
			return;
	...
	req.on('httpUploadProgress', function (data) {
		_this.emit('partProgress', data);
	});
};

Multipart.prototype._handleSort = function _handleSort(a, b) {
	return a.PartNumber < b.PartNumber ? -1 : 1;
};

Multipart.prototype._handlePendingQueue = function _handlePendingQueue() {
...
Multipart.prototype._createPart = function _createPart(part, callback) {
	var params,
		_this = this;
...
Multipart.prototype.abort = function abort() {
	var params,
		_this = this;
...
Multipart.prototype.pause = function pause() {
	this.queue.pause();
};

Multipart.prototype.resume = function resume() {
	this.queue.resume();
};
